...
class PlayerStatusServerModule(BaseServerModule):
    ...
    def example(self, system):
        system.addData(
            StaticConfig.Data.STATUS_DETAIL,
            {
                "mana": {
                    "default_now": 0,  # 默认初始值
                ...
                "exp": {
                    ...
                    "transfer": {  # 选填，自动升级
                        ...
                        "from_now": lambda now, max: max
                        - now,  # 起始数值的变化量，此配置为做差，也可以直接配置变成0
                        "from_max": lambda max, tnow: tnow * 10
                        + 30,  # 起始数值的新最大值,max为当前最大值，tnow为目标值的当前值（尚未进行to_amount操作）
                    },
                    "recover": {  # 选填，自动回复
                        "period": 1,  # 多少秒进行一次恢复
                        "multiple": 0,  # 恢复的百分比
