...
class ArmorModule(BaseServerModule):
    ...
    def example(self, system):
        ...
        # 攻击特效
        system.addData(StaticConfig.Data.ARMOR_EFFECT, {
            "minecraft:diamond_sword": [
                # 对自己发送指令
                ("cmd", {"cmd": "playsound @a player.hurt"}),
                # 对自己播放序列帧
                ("sfx_source", {"path": "sfx/1", "faceCamera": False, "bindRot": (0, 90, 0)}),
                # 对目标播放序列帧
                ("sfx_target", {"path": "sfx/2", "scale": (3, 3, 3)}),
                # 对目标添加药水效果
                ("buff_target", [("poison", 15, 1, False), ]),
                # 引燃目标
                ("fire_target", 10),
            ],
        }, version=0)
        # 持续获得的buff
        system.addData(StaticConfig.Data.ARMOR_BUFF, {
            "minecraft:diamond_sword": [
                # buff名、时长、等级、显示粒子
                ("speed", 15, 1, False),
        ...
        system.addData(StaticConfig.Data.ARMOR_PARTICLE, {
            # 盔甲的粒子
            "minecraft:diamond_sword": [
                {"type": "minecraft_particle",  # 粒子类型，可选值：minecraft_particle（原版粒子）、netease_particle（产品粒子）、netease_sfx（产品序列帧）
                 "args": ("minecraft:xjcraft_wire_dust_particle",  # 粒子名
                          lambda tick, isMoving: isMoving and tick % 1 == 0,  # tick是是否显示
                          lambda x, y, z: (x - 0.1, y - 1.2, z))},  # 显示坐标
                {"type": "minecraft_particle",
                 "args": ("minecraft:xjcraft_wire_dust_particle",
        ...
        # 免疫的buff
        system.addData(StaticConfig.Data.ARMOR_IMMUNITY_BUFF, {
            "minecraft:diamond_sword": {
                "blindness"
            },
            ("minecraft:diamond_helmet", "minecraft:diamond_chestplate", "minecraft:diamond_leggings",
        ...
        system.addData(StaticConfig.Data.ARMOR_IMMUNITY_DAMAGE, {
            "minecraft:diamond_sword": {
                "*"
            },
            ("minecraft:diamond_helmet", "minecraft:diamond_chestplate", "minecraft:diamond_leggings",
        ...
        system.addData(StaticConfig.Data.ARMOR_SUCK_BLOOD, {
            "minecraft:diamond_sword": lambda damage: max(damage * 0.5, 1),
        }, version=0)
        # 秒杀
        system.addData(StaticConfig.Data.ARMOR_KILL, {
            "minecraft:diamond_sword": {},
        }, version=0)
        # 盔甲增伤
        system.addData(StaticConfig.Data.ARMOR_DAMAGE_BOOST, {
            "minecraft:diamond_helmet": lambda damage: max(damage, 7),
        }, version=0)
        # 反伤，后面是伤害与加血的数值关系
        system.addData(StaticConfig.Data.ARMOR_THRONE, {
            "minecraft:diamond_helmet": lambda damage: max(damage * 0, 1),
        }, version=0)
        # 属性切换,可用属性只有HEALTH与SPEED,
        # 添加了JUMP属性用于更新客户端二段跳,使用此功能必须在客户端添加二段跳模组
        system.addData(StaticConfig.Data.ARMOR_ATTR, {
        ...
        # 免疫仇恨
        system.addData(StaticConfig.Data.ARMOR_RESET_TARGET, {
            ("minecraft:diamond_helmet", "minecraft:diamond_chestplate", "minecraft:diamond_leggings",
             "minecraft:diamond_boots",): {
                "zombie"
            }
        ...
        # 受到伤害执行指令
        system.addData(StaticConfig.Data.ARMOR_SET_COMMAND, {
            "minecraft:diamond_helmet": {
                "durability": 15,  # 扣除耐久值
                "command": "/spreadplayers ~ ~ 10 20 @s"},  # 执行的指令
        }, version=0)
        # 减伤，后面是伤害与减伤的数值关系
        system.addData(StaticConfig.Data.ARMOR_REDUCE_DAMAGE, {
            "minecraft:diamond_helmet": lambda damage: max(damage - 4, 1),
        }, version=0)
        # 每隔一段时间，增加耐久
        system.addData(StaticConfig.Data.ARMOR_ADD_DURABILITY, {
            "minecraft:diamond_sword": {
                "duration": 2,  # 时间间隔，10秒的整数倍，2意味着20秒
                "add_durability": 10,  # 给所有装备增加的耐久值
                "durability": 20},  # 扣除此装备的耐久值
        }, version=0)
