...
- 处理多个组件之间代码的覆盖问题，可以合并所有同名模块与配置，使用最新的版本，轻松实现主/子包，扩展包的功能
- 清晰的模块化架构，模块之前划分清晰
- 封装了大量ui控件操作，完整的长按拖动——记录位置功能（参考EquipmentDisplayModule）
- 采用装饰器封装了事件的监听，再也不用写冗长的listen功能
- 采用装饰器封装了面向对象的事件@Event.Wrapper，可以通过注解轻松的使用面向对象进行事件处理
- 封装了大量实用功能（由前置包提供，框架中提供了接口，未来将会出一些教程）
...
有些具体实现，暂未开源，会以前置的方式发布,如需使用，请提示用户搜索并加载免费组件：《小鸡万能模板前置》
